...
barplot(medianyear_top10ussr[,2] - median(medianyear_top10ussr[,2]),xlab = "Страна", ylab = "Отклонение медианы от общей медианы, лет",names.arg=medianyear_top10ussr[,1],ylim=c(-2,2))
...
barplot(meanage[,2] - mean(meanage[,2]),xlab = "Страна", ylab = "Отклонение от среднего возраста, лет",names.arg=meanage[,1],ylim=c(-3,3))


meanage_top10ussr = aggregate(2013-top10ussr$YEAR, by=list(top10ussr$CNTRY), FUN=mean)
barplot(meanage_top10ussr[,2] - mean(meanage[,2]),xlab = "Страна", ylab = "Отклонение от среднего возраста, лет",names.arg=meanage_top10ussr[,1],ylim=c(-1,1))
